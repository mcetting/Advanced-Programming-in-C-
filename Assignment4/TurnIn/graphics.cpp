...
void window::keyboard(GLubyte key, int x, int y)
{
   ...
   case 'H':
   case 'h':
      move_selected_object(-move_speed, 0);
      break;
   case 'J':
   case 'j':
      move_selected_object(0, -move_speed);
      break;
   case 'K':
   case 'k':
      move_selected_object(0, move_speed);
...
{
   window::objects.at(window::selected).move(x, y);
}
